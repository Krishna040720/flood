...
## Hourly watcher — trusted sources auto-publish, everything else waits for review

Set `ADMIN_TOKEN` (see `.env.example`) to enable this. Every hour, the
server checks the feeds in `sources.json` for articles that mention a
donation or a CMRF total. What happens next depends on the source:

- **From a domain listed in `trusted-sources.json`**, with a complete
  extraction (donor + amount + date, or amount + date for a total) —
  **published immediately**, no human involved. Every auto-publish is
  logged to `auto-published-log.jsonl` (viewable via
  `GET /api/admin/auto-published`), so "automatic" never means "invisible."
- **Anything else** — general news, blogs, an incomplete extraction even
  from a trusted domain — **queued** in `pending-entries.json` for you to
  review and approve.

Why the split instead of one or the other: fully automatic publishing
from *any* source risks attributing a wrong amount to a real donor from
an unverified article — a real accuracy/reputational risk on a public
ledger. Fully manual review of *everything*, including the CMRF's own
official announcements, is unnecessary friction. Keeping
`trusted-sources.json` short and genuinely authoritative (government
domains, the fund's own site) gets you the automatic feel where it's
safe, and a review gate everywhere it isn't.

**Review workflow** for the queue, using the `x-admin-token` header (or
`?token=` query param) set to your `ADMIN_TOKEN`:

```
# See what's waiting for review
GET /api/admin/pending

# Approve one — the watcher's guessed donor/amount/date can be
# overridden by passing corrected fields in the JSON body
POST /api/admin/pending/<id>/approve
  { "donor": "Corrected Name", "amountCr": 2.5 }   # optional overrides

# Reject one — removes it, nothing gets published
DELETE /api/admin/pending/<id>

# Trigger a check right now instead of waiting for the hourly timer
POST /api/admin/watch/run

# Audit trail of everything auto-published without review
GET /api/admin/auto-published
```

**Render free tier note:** the in-process hourly timer only runs while
the instance is awake, and free instances spin down after ~15 minutes of
inactivity. For a reliable hourly cadence, point a free external pinger
(e.g. cron-job.org, UptimeRobot) at `POST /api/admin/watch/run` with the
`x-admin-token` header, once an hour, instead of relying on the built-in
timer alone.

**Tune `sources.json`** to add/remove feeds, and **`trusted-sources.json`**
to add/remove auto-publish domains. Keep the trusted list short — it ships
with just `cm.assam.gov.in`, `assam.gov.in`, and `pib.gov.in` as a starting
point. Anything you add there can land on the public ledger unattended,
so only add domains you'd trust with that.
