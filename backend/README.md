...
## Hourly watcher — finds candidates, never auto-publishes
...
donation or a CMRF total, and queues anything it finds as a **candidate**
in `pending-entries.json` — it never writes to the public ledger or
`official-data.json` on its own. A human has to review and approve each
one. This is intentional: the whole point of this tracker is that every
figure traces to a verified source, and an unattended scraper publishing
straight to a hash-chained public ledger would eventually attribute a
wrong amount to a real donor.

**Review workflow**, using the `x-admin-token` header (or `?token=` query
param) set to your `ADMIN_TOKEN`:
...
**Tune `sources.json`** to add/remove feeds — it ships with a couple of
broad Google News RSS searches as a starting point. Narrower, more
trusted feeds (specific outlets, the CMRF's own updates page if it ever
gets an RSS feed) will produce fewer false positives than a broad search.
